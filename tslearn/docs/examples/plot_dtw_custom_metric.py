...
# Create the axes and plot the distance matrices
plt.figure(1, figsize=(12, 6))
axs_s_x, axs_s_y, axs_dist = [], [], []
for i in range(2):
    rect_s_y = [left[i], bottom, w_ts, height]
    rect_dist = [left_h[i], bottom, width, height]
    rect_s_x = [left_h[i], bottom_h, width, h_ts]

    ax_dist = plt.axes(rect_dist)
    ax_s_x = plt.axes(rect_s_x)
    ax_s_y = plt.axes(rect_s_y)

    ax_dist.imshow(distances[i], origin='lower')
    ax_dist.axis("off")
    ax_dist.autoscale(False)
    ax_dist.plot(*zip(*paths[i]), "w-", linewidth=3.)

    ax_s_x.axis("off")
    ax_s_y.axis("off")

    axs_dist.append(ax_dist)
    axs_s_x.append(ax_s_x)
    axs_s_y.append(ax_s_y)


# Plot the time series for example 1
ticks_location = [-pi, 0, pi]
ticks_labels = [r"$\bf-\pi$", r"$\bf0$", r"$\bf\pi$"]

axs_s_x[0].plot([0, sz - 1], [ticks_location]*2, "k--", alpha=.2)
axs_s_x[0].plot(np.arange(sz), dataset_scaled_1[1], "b-", linewidth=3.)
axs_s_x[0].set_xlim((0, sz - 1))

axs_s_y[0].plot([ticks_location]*2, [0, sz - 1], "k--", alpha=.2)
axs_s_y[0].plot(-dataset_scaled_1[0], np.arange(sz), "b-", linewidth=3.)
axs_s_y[0].set_ylim((0, sz - 1))

for loc, s in zip(ticks_location, ticks_labels):
    axs_s_x[0].text(0, loc, s, fontsize="large", color="grey",
                    horizontalalignment="right", verticalalignment="center")
    axs_s_y[0].text(-loc, 0, s, fontsize="large", color="grey",
                    horizontalalignment="center", verticalalignment="top")

# Plot the time series for example 2
colors = [(1, 1, 1), (0, 0, 1)]  # White -> Blue
cmap_name = 'white_blue'
cm = LinearSegmentedColormap.from_list(cmap_name, colors, N=2)

axs_s_x[1].imshow(data[1][1].T, aspect="auto", cmap=cm)
axs_s_y[1].imshow(np.flip(data[1][0], axis=1), aspect="auto", cmap=cm)
