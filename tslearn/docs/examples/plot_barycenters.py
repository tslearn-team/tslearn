...
plt.subplot(3, 1, 2)
dba_bar = dtw_barycenter_averaging(X, max_iter=100, verbose=False, init_barycenter=numpy.zeros((20, 1)))
for ts in X:
    plt.plot(ts.ravel(), "k-", alpha=.2)
...
plt.subplot(3, 1, 3)
sdtw_bar = softdtw_barycenter(X, gamma=1., max_iter=100, init=numpy.zeros((20, 1)))
for ts in X:
    plt.plot(ts.ravel(), "k-", alpha=.2)
