...
"""
==========================================================
Unsupervised Nearest Neighbors Search  (Chinatown dataset)
==========================================================
...
#
# The dataset consists of pedestrian counts recorded at the Chinatown location in Melbourne, Australia. 
# Each time series represents the number of pedestrians detected during a day, weekday or weekend. 
#
...
# one where all neighbors belong to class 0 (Weekend) and another where all neighbors belong
# to class 1 (Weekday).
import numpy as np
...
for i, idx in enumerate([ind_0, ind_1]):
    ...
    plt.ylabel(f"Pedestrian count (scaled)")
    plt.legend()

plt.suptitle("Nearest Neighbors for Test Time Series\nMelbourne's Chinatown Pedestrian Count")
plt.xlabel("Time")
plt.tight_layout()
plt.show()
...
# represent the "Weekend" pedestrian movement pattern. The test time series and its neighbors share
# a characteristic higher night activity.
# 
# In the bottom subplot, we see the nearest neighbors from the "Weekday" movement class.
# Notably, these neighbors are identified as nearest despite some shift in peak times,
